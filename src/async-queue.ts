import { ClassLogger } from './class-logger' // Corrected import
...
export class AsyncQueue {
  ...
  constructor () {
    this._logger = new ClassLogger('AsyncQueue')
    this._logger.debug('AsyncQueue initialized')
  }
  ...
  private _executeNext (): void {
    ...
    // Update logger with current queue size
    this._logger.setAdditionalContext({ queueSize: this._queue.length })
    this._logger.debug('Executing next task', { functionName: '_executeNext' })
    ...
    try {
      task.execute(() => {
        ...
        // Update logger with new queue size
        this._logger.setAdditionalContext({ queueSize: this._queue.length })
        this._logger.debug('Task completed', { functionName: '_executeNext' })
