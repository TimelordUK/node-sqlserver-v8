const sql = require('msnodesqlv8/lib/sql.js')
const os = require('os')
...
class StressTestRunner {
  ...
  logMemory(message = '') {
    ...
    if (this.memoryBaseline) {
      ...
      console.log(`  RSS Delta: ${rssDelta > 0 ? '+' : ''}${rssDelta} MB`)
    }
    console.log('')
  }
  ...
  async runScenario(scenario) {
    ...
    for (let i = 0; i < this.config.iterations; i++) {
      try {
        ...
        if (global.gc && i > 0 && i % this.config.gcInterval === 0) {
          global.gc()
          await new Promise(resolve => setTimeout(resolve, 10))
        }

      } catch (err) {
        this.errors.push({ iteration: i, error: err })
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    ...
    Object.entries(runner.scenarios).forEach(([key, scenario]) => {
