...
var TestEnv = require('./test-env').TestEnv;
...
var PrintConnection = /** @class */ (function () {
    ...
    PrintConnection.prototype.test = function (conn_str, conn, done) {
        conn.query("select @@SPID as id, CURRENT_USER as name", function (err, res) {
            var sp = res[0]['id'];
            console.log("open[".concat(sp, "]:  ").concat(conn_str));
            conn.query(getConnectionsSql, function (err, res) {
                var count = res[0]['NumberOfConnections'];
                conn.close(function () {
                    console.log("close[".concat(sp, "]: NumberOfConnections = ").concat(count));
                    done();
                });
            });
        });
...
var Benchmark = /** @class */ (function () {
    ...
    Benchmark.prototype.run = function (conn_str, argv) {
        ...
        var top = argv.top || -1;
        var query = top < 0 ?
            "select ".concat(columns, " from ").concat(schema, ".").concat(table) :
            "select top ".concat(top, " ").concat(columns, " from ").concat(schema, ".").concat(table);
        console.log("Benchmark query ".concat(query));
        var runs = 0;
        var total = 0;
        var statement = null;
        ...
        get_ready(function (err, conn, ps) {
            ...
            function once(d, err, rows) {
                ...
                ++runs;
                total += elapsed;
                console.log("[".concat(table, "\t] rows.length ").concat(rows.length, " \t elapsed ").concat(elapsed, "\t ms [ runs ").concat(runs, " avg ").concat(total / runs, " ]"));
                if (runs == repeats) {
                    clearInterval(repeatId);
                    if (prepared) {
    ...
    ProcedureOut.prototype.run = function (conn_str, argv) {
        ...
        sql.open(conn_str, function (err, theConnection) {
            ...
            var def = "alter PROCEDURE <name>(\n@id INT,\n@name varchar(20) OUTPUT,\n@company varchar(20) OUTPUT\n\n)AS\nBEGIN\n" +
                "   SET @name = '".concat(s1, "'\n") +
                "   SET @company = '".concat(s2, "'\n") +
                '   RETURN 99;\n' +
                'END\n';
            var x = 0;
            env.procedureHelper.createProcedure(spName, def, function () {
                setInterval(function () {
                    var pm = theConnection.procedureMgr();
                    pm.callproc(spName, [1], function (err, results, output) {
                        assert.ifError(err);
                        var expected = [99, s1, s2];
                        console.log("".concat(JSON.stringify(output), " x = ").concat(x++));
        ...
        sql.open(conn_str, function (err, conn) {
            ...
            setInterval(function () {
                ...
                var q = conn.query(qs, function (err, results, more) {
                    console.log("[".concat(x, "] more = ").concat(more, " err ").concat(err, " expected ").concat(expected, " results ").concat(results[0].test_field));
                    assert.deepEqual(results[0].test_field, expected);
                    if (more)
                        return;
                    console.log("[".concat(x, "] completes more = ").concat(more));
                    ++x;
                });
                q.on('msg', function (err) {
                    console.log("[".concat(x, "]: q.msg = ").concat(err.message));
                });
            }, delay);
        });
    };
        ...
        sql.open(conn_str, function (err, conn) {
            ...
            setInterval(function () {
                ...
                for (var i = 0; i < repeats; ++i) {
                    qs += "RAISERROR('[".concat(x, "]: Error Number ").concat(i + 1, "', 1, 1);");
                }
                var q = conn.query(qs, function (err, results, more) {
                    if (more && !err && results && results.length === 0) {
                        return;
                    }
                    console.log("[".concat(x, "] more = ").concat(more, " err ").concat(err, " results ").concat(JSON.stringify(results)));
                    if (more)
                        return;
                    console.log("[".concat(x, "] completes more = ").concat(more));
                    ++x;
                ...
                q.on('info', function (err) {
                    console.log("[".concat(x, "]: q.info = ").concat(err.message));
                });
            }, delay);
        });
    };
...
var BusyConnection = /** @class */ (function () {
    ...
    BusyConnection.prototype.run = function (conn_str, argv) {
        ...
        sql.open(conn_str, function (err, conn) {
            ...
            var x = 1;
            setInterval(function () {
                var query = "RAISERROR('User JS Error ".concat(severity, "', ").concat(severity, ", 1);SELECT ").concat(x, "+").concat(x, ";");
                console.log(query);
...
var LargeStringSelect = /** @class */ (function () {
    ...
    LargeStringSelect.prototype.run = function (conn_str, argv) {
        ...
        sql.open(conn_str, function (err, conn) {
            ...
            var query = "SELECT 'Result' AS [Result], '".concat(p, "' AS [ReallyLongString]");
            setInterval(function () {
                ...
                q.on('row', function (row) {
                    console.log("[column:".concat(x, "]: row = ").concat(row));
                });
                q.on('column', function (col, data, more) {
                    console.log("[column:".concat(x, "]: col = ").concat(col, " data.length ").concat(data.length, ", more : ").concat(more, " p.length ").concat(p.length));
                });
            }, delay);
        });
    };
...
var PrintSelect = /** @class */ (function () {
    ...
    PrintSelect.prototype.run = function (conn_str, argv) {
        ...
        sql.open(conn_str, function (err, conn) {
            ...
            setInterval(function () {
                conn.query("print 'JS status message ".concat(x, "'; SELECT ").concat(x, " + ").concat(x, " as res;SELECT ").concat(x, " * ").concat(x, " as res2"), function (err, results, more) {
                    if (more && !err && results && results.length === 0) {
                        return;
                    }
                    console.log("[".concat(x, "] more = ").concat(more, " err ").concat(err, " results ").concat(JSON.stringify(results)));
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, this.promised(conn, "SELECT ".concat(x, "+").concat(x, ";"))];
                    case 2:
                        results = _a.sent();
                        if (iteration % 1000 === 0) {
                            console.log("iteration = ".concat(iteration, " out of ").concat(iterations));
                            console.log(results);
                        }
                        return [3 /*break*/, 1];
...
switch (argv.t) {
    ...
    case "procedure":
        ...
        break;
    default:
        console.log("test ".concat(argv.t, " is not valid."));
        break;
}
if (test != null) {
    env.open().then(function () {
