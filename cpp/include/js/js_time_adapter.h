...
class JSTimeAdapter {
 public:
  ...
  static bool bindJsDateToDateStorage(napi_env env,
                                      napi_value jsDate,
                                      DatumStorage& storage,
                                      int32_t offset = 0);

  // Bind a JavaScript Date to a TIME type in DatumStorage
  static bool bindJsDateToTimeStorage(napi_env env,
                                      napi_value jsDate,
                                      DatumStorage& storage,
                                      int32_t offset = 0);

  // Bind a JavaScript Date to a TIMESTAMP type in DatumStorage
  static bool bindJsDateToTimestampStorage(napi_env env,
                                           napi_value jsDate,
                                           DatumStorage& storage,
                                           int32_t offset = 0);

  // Bind a JavaScript Date to a TIMESTAMPOFFSET type in DatumStorage
  static bool bindJsDateToTimestampOffsetStorage(napi_env env,
                                                 napi_value jsDate,
                                                 DatumStorage& storage,
                                                 int32_t offset = 0);
  ...
  static napi_value createJsDateFromTimestampOffset(
      napi_env env, const SQL_SS_TIMESTAMPOFFSET_STRUCT& offsetTimestamp);
};
