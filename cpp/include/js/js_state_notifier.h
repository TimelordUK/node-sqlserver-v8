...
class JsStateNotifier : public IOdbcStateNotifier {
 public:
  ...
  void OnStateChange(int statementId, 
    ...
    auto callback = [statementId, oldState, newState](Napi::Env env, Napi::Function jsCallback) {
      jsCallback.Call({
        Napi::Number::New(env, statementId),
        Napi::String::New(env, OdbcStatementStateToString(oldState)),
        Napi::String::New(env, OdbcStatementStateToString(newState))
      });
    };
