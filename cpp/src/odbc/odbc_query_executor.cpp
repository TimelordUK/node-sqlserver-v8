...
OdbcQueryExecutor::OdbcQueryExecutor(std::shared_ptr<IOdbcApi> api,
    ...
    : _api(api), _connectionHandles(connectionHandles), _errorHandler(errorHandler) {}

bool OdbcQueryExecutor::ExecuteQuery(const std::string& sqlText,
...
bool OdbcQueryExecutor::BindParameters(
    IOdbcStatementHandle* stmt, const std::vector<std::shared_ptr<QueryParameter>>& parameters) {
...
bool OdbcQueryExecutor::ProcessResults(IOdbcStatementHandle* stmt,
                                       std::shared_ptr<QueryResult>& result) {
  ...
  for (SQLSMALLINT i = 1; i <= numCols; i++) {
    ...
    ret = SQLDescribeCol(stmt->get_handle(),
                         i,
                         colDef.colName,
...
SQLRETURN OdbcQueryExecutor::GetColumnData(SQLHSTMT hStmt,
                                           SQLSMALLINT colNum,
