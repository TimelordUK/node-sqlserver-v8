...
SQLSMALLINT parseOdbcParamTypeString(const std::string& typeStr) {
  if (typeStr.empty()) {
 ...
 */
SQLSMALLINT parseOdbcTypeString(const std::string& typeStr) {
 ...
 */
SQLSMALLINT getOdbcCTypeFromJsType(int jsType) {
  switch (jsType) {
    case 0:  // JS_UNKNOWN
      return SQL_C_DEFAULT;
    case 1:  // JS_NULL
 ...
 */
SQLSMALLINT getOdbcSqlTypeFromJsType(int jsType) {
  switch (jsType) {
    case 0:  // JS_UNKNOWN
      return SQL_UNKNOWN_TYPE;
    case 1:  // JS_NULL
...
bool OdbcStatement::bind_parameters(const std::vector<std::shared_ptr<SqlParameter>>& parameters) {
  ...
  for (const auto& datum : parameters) {
    ...
    const auto c_type = parseOdbcTypeString(datum->c_type);
    const auto sql_type = parseOdbcTypeString(datum->sql_type);
