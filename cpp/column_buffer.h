...
// Contents: Buffer for column data in ODBC operations
//
...
namespace mssql
{
  ...
  public:
    ...
    {
      SQL_LOG_TRACE_STREAM("Creating ColumnBuffer - Type: " << dataType
                                                            << ", Size: " << columnSize);
      buffer_.resize(columnSize);
    }

    // Get the buffer pointer for SQLBindCol
    SQLPOINTER getBuffer() { return buffer_.data(); }

    // Get the buffer length
    SQLLEN getBufferLength() const { return static_cast<SQLLEN>(buffer_.size()); }

    // Get pointer to the indicator array
    SQLLEN *getIndicator() { return indicators_.data(); }

    // Set the row array size for batch fetching
    void setRowArraySize(size_t size)
    {
      indicators_.resize(size);
      if (dataType_ == SQL_CHAR || dataType_ == SQL_VARCHAR ||
          dataType_ == SQL_WCHAR || dataType_ == SQL_WVARCHAR ||
          dataType_ == SQL_BINARY || dataType_ == SQL_VARBINARY)
      {
    ...
    std::shared_ptr<DatumStorage> getValue(size_t rowIndex)
    {
      ...
      case SQL_TINYINT:
        ...
        break;
      case SQL_SMALLINT:
        storage_->addValue<int16_t>(*reinterpret_cast<int16_t *>(&buffer_[offset * sizeof(int16_t)]));
        break;
      case SQL_INTEGER:
        storage_->addValue<int32_t>(*reinterpret_cast<int32_t *>(&buffer_[offset * sizeof(int32_t)]));
        break;
      case SQL_BIGINT:
        storage_->addValue<int64_t>(*reinterpret_cast<int64_t *>(&buffer_[offset * sizeof(int64_t)]));
        break;
      case SQL_REAL:
      case SQL_FLOAT:
      case SQL_DOUBLE:
      ...
      case SQL_DECIMAL:
      case SQL_NUMERIC:
        storage_->addValue<SQL_NUMERIC_STRUCT>(*reinterpret_cast<SQL_NUMERIC_STRUCT *>(&buffer_[offset * sizeof(SQL_NUMERIC_STRUCT)]));
        break;
      case SQL_TYPE_DATE:
        storage_->addValue<SQL_DATE_STRUCT>(*reinterpret_cast<SQL_DATE_STRUCT *>(&buffer_[offset * sizeof(SQL_DATE_STRUCT)]));
        break;
      case SQL_TYPE_TIME:
        storage_->addValue<SQL_SS_TIME2_STRUCT>(*reinterpret_cast<SQL_SS_TIME2_STRUCT *>(&buffer_[offset * sizeof(SQL_SS_TIME2_STRUCT)]));
        break;
      case SQL_TYPE_TIMESTAMP:
        storage_->addValue<SQL_TIMESTAMP_STRUCT>(*reinterpret_cast<SQL_TIMESTAMP_STRUCT *>(&buffer_[offset * sizeof(SQL_TIMESTAMP_STRUCT)]));
        break;
      case SQL_CHAR:
      case SQL_VARCHAR:
      case SQL_WCHAR:
    ...
    {
      SQL_LOG_TRACE_STREAM("Setting data for row " << rowIndex
                                                   << " - Type: " << dataType
                                                   << ", Size: " << columnSize);
