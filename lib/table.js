...
const tableModule = (() => {
    ...
    constructor (connection, connectionMeta, connectionUser, sharedCache) {
      ...
      function getUserTypeTable (name, callback) {
        const mapFn = sql => {
          ...
          if (schemaIndex > 0) {
            schemaName = name.substring(0, schemaIndex)
            unqualifiedTableName = name.substring(schemaIndex + 1)
          }
  ...
  class Meta {
    ...
    recalculateAssignableColumns () {
      return this.allColumns.filter(col => !col.readOnly())
    }
  ...
  class BulkTableOpMgr {
    constructor (theConnection, user, m) {
      ...
      function userTypeCols () {
        ...
        columns.forEach(col => {
          const declaration = `${col.typed(true)}`
          let length = 0
          if (col.max_length > 0) {
      ...
      function prepare () {
        ...
        !summary.by_name[property].readOnly()) {
