  ...
  class TableMgr {
    constructor (connection, connectionMeta, connectionUser, sharedCache) {
      ...
      */
      ...
      function makeParam (tableName, tableSchema, position, paramName, paramType, paramLength, isPrimaryKey) {
        return new TableParam(tableName, tableSchema, position, paramName, paramType, paramLength, isPrimaryKey)
      }

      function asTableParam (rawCol) {
        const tp = new TableParam()
        tp.fromRaw(rawCol)
        return tp
      }
        ...
        return new Promise((resolve, reject) => {
          ...
          if (!tableMeta) {
            describeTable(name).then(rawCols => {
              const tps = rawCols.map(rc => asTableParam(rc))
              tableMeta = addTable(name, tps)
              resolve(tableMeta)
            }).catch(err => {
              reject(err)
            })
