...
const tableModule = (() => {
  ...
  class TableBuilder {
    ...
    setDialect (d) {
      this.dialect = d
      if (d === ServerDialect.Sybase) {
        this.tableCatelog = null
    ...
    enclose (txt) {
      if (!txt) return null
      const begin = this.dialect === ServerDialect.SqlServer ? '[' : ''
      const end = this.dialect === ServerDialect.SqlServer ? ']' : ''
      return `${begin}${txt}${end}`
    }

    getFullName () {
      const tableCatelog = this.enclose(this.tableCatelog)
      const tableSchema = this.enclose(this.tableSchema)
      const tableName = this.enclose(this.tableName)
    ...
    compute () {
      ...
      if (this.dialect === ServerDialect.Sybase) {
        dropTableSql = `IF OBJECT_ID('${tableName}') IS NOT NULL DROP TABLE ${tableName};`
      } else {
