...
const tableModule = (() => {
  ...
  class TableBuilder {
    constructor (mgr, conn, tableName, tableCatelog, tableSchema) {
      ...
      if (this.tableCatelog.length > 0) {
        this.fullTableName = `[${this.tableCatelog}].[${this.tableSchema}].[${this.tableName}]`
      } else {
        this.fullTableName = `[${this.tableSchema}].[${this.tableName}]`
      }
    ...
    typed (c) {
      ...
      return isSized ? `${c.type} (${c.max_length})` : `${c.type}`
    }
    ...
    toTable () {
      this.compute()
      const table = this.mgr.addTable(this.tableName, this.columns)
  ...
  class TableMgr {
    constructor (connection, connectionMeta, connectionUser, sharedCache) {
      ...
      */
      ...
      function addMeta (tableName, columns) {
        const tableMeta = new Meta(tableName, columns)
        cache[tableMeta] = tableMeta
        return tableMeta
      }

      function addTable (tableName, columns) {
        const meta = addMeta(tableName, columns)
