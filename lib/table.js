  ...
  function TableMgr (connection, connectionMeta, connectionUser) {
    ...
    function Meta (tableName, cols) {
      ...
      function whereClause (colSubSet) {
        let w = colSubSet.map(e => `[${e.name}] = ?`).join(' and ')
        return `where ( ${w} )`
      }
      ...
      function selectStatement (colSubSet) {
        return `select ${columnsSql(allColumns)} from ${fullTableName} ${whereClause(colSubSet)}`
      }
      ...
      function updateStatement (colSubSet) {
        let w = colSubSet.map(e => `[${e.name}] = ?`).join(' and ')
        return `update ${fullTableName} set ${w} ${whereClause(whereColumns)}`
      }

      function insertStatement () {
        const subSet = recalculateAssignableColumns()
        let w = subSet.map(() => `?`).join(', ')
        let sql = `insert into ${fullTableName} ( ${columnsSql(subSet)} )`
        if (subSet.length > 0) {
          sql += ` values ( ${w} )`
        }
      ...
      function setWhereCols (colSubSet) {
        let subSet = filteredSet(colSubSet)
        whereColumns = subSet
      ...
      function setUpdateCols (colSubSet) {
        let filtered = filteredSet(colSubSet)
      ...
      function recalculateUpdateColumns () {
        ...
        return assignable.filter(col => !primaryByName.hasOwnProperty(col.name))
      }
