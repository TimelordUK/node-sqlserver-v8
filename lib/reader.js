...
var readerModule = (function () {
  function DriverRead (cppDriver, queue) {
    ...
    /* route non critical warning messages to its own event to prevent streams based readers from halting */
    function routeStatementError (err, callback, notify, more) {
      var warning = err && err.sqlstate && err.sqlstate.length >= 2 && err.sqlstate.substring(0, 2) === '01'
      if (callback && !warning) {
        callback(err, null, more)
      } else {
        var ev = warning ? 'warning' : 'error'
        if (notify && notify.listeners(ev).length > 0) {
          notify.emit(ev, err)
        } else {
          throw new Error(err)
        }
    ...
    function fetch (notify, query, params, invokeObject, callback) {
      ...
      function onReadColumnMore (err, results) {
        setImmediate(function queuedOnReadColumnMore () {
          ...
          if (!more ) {
            notify.emit('partial-column', column, data, more)
      ...
      function onReadColumn (err, results, more) {
