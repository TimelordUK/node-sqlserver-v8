...
const readerModule = ((() => {
  function DriverRead (cppDriver, queue) {
    ...
    function Query (notify, query, params, invokeObject, callback) {
      ...
      function close () {
        notify.removeListener('pause', pause)
        notify.removeListener('resume', resume)
        running = false
        workQueue.nextOp()
      }
      ...
      const moveToNextResult = async (nextResultSetInfo) => {
        setImmediate(async () => {
          if (!meta) {
            rowsCompleted(
              {
      ...
      async function dispatch () {
        try {
          let dispatching = running
          while (dispatching) {
            const last = await nativeGetRows(queryId, rowBatchSize)
            dispatchRows(last)
            if (paused) {
              break
            }
            dispatching = !last.end_rows
          }
          await nextResult()
        } catch (err) {
      ...
      notify.on('pause', pause)
      notify.on('resume', resume)
    ...
    const fetch = (notify, query, params, invokeObject, callback) => {
      const f = new Query(notify, query, params, invokeObject, callback)
      f.begin()
    }
