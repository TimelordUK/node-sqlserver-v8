...
class Query extends BasePromises {
      ...
      this.queryHandler.begin(queryId, this.query, this.params, (e, queryResult, procOutputOrMore) => {
        ...
        setImmediate(() => {
          if (e && queryResult.endOfResults && queryResult.endOfRows) {
            ...
            logger.debugLazy(() => `beginQuery error, no more results, ending query ${this.queryId}`, 'DriverRead.beginQuery')
            ...
            if (queryResult.handle && queryResult.handle.statementId >= 0) {
              this.queryHandler.end(this.notify, this.outputParams, () => {
                logger.debugLazy(() => `query handler end ${this.queryId} - error count ${msgs.length}`)
                reject(msgs)
              }, null, false)
            } else {
              reject(msgs)
  ...
  moveToNextResult (nextResultSetInfo) {
    setImmediate(() => {
      if (this.cancelled) {
        this.running = false
        logger.debugLazy(() => `query ${this.queryId} has been cancelled - ending query ${this.queryId}`)
        this.end()
        return
      }
  ...
  dispatch () {
    ...
    this.nativeGetRows(this.queryId, this.rowBatchSize).then(d => {
      logger.traceLazy(() => `dispatch received ${d?.data?.length || 0} rows for queryId ${this.queryId}, endOfRows=${d?.endOfRows} endOfResults=${d?.endOfResults}`, 'DriverRead.dispatch')
      this.batchRowIndex = 0
      this.batchData = d
      this.dispatchRows(d)
      if (!d.endOfRows) {
  ...
  begin () {
    this.beginQuery(this.queryId, this.query, this.params).then(res => {
        ...
        if (!this.cancelled) {
          res.warning.forEach(err => {
            if (!this.cancelled) {
              ...
              if (this.cancelled) {
                logger.debugLazy(() => `statement has been cancelled queryId ${this.queryId}`)
              }
            }
          })
        }
      ...
      logger.debugLazy(() => `dispatch error for queryId ${this.queryId}: ${err}`)
  ...
  setupTimeoutHandling () {
    ...
    if (timeoutSecs > 0) {
      ...
      if (useJsCancel) {
        ...
        logger.debugLazy(() => `Using JS-based cancel for Linux driver v17, timeout: ${timeoutMs}ms (${timeoutSecs}s)`, 'Query.setupTimeoutHandling')
        ...
        logger.debugLazy(() => `Setting up immediate timeout: ${timeoutMs}ms`, 'Query.setupTimeoutHandling')
        ...
        logger.debugLazy(() => `Using native driver timeout: ${timeoutSecs} seconds`, 'Query.setupTimeoutHandling')
