...
class Query extends BasePromises {
  ...
  dispatchRows (results) {
    ...
    if (global.msnodesqlv8Logger && global.msnodesqlv8Logger.isLevelEnabled('TRACE')) {
      global.msnodesqlv8Logger.log('TRACE', `[reader.dispatchRows] Processing ${numberRows} rows`)
      if (numberRows > 0 && resultRows[0]) {
        global.msnodesqlv8Logger.log('TRACE', `[reader.dispatchRows] First row has ${resultRows[0].length} columns`)
        if (resultRows[0][0] && typeof resultRows[0][0] === 'string') {
          global.msnodesqlv8Logger.log('TRACE', `[reader.dispatchRows] First column length: ${resultRows[0][0].length}`)
        }
      }
    }
