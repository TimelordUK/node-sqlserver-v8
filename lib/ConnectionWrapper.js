...
function ConnectionWrapper (qu, defCb, nat, name) {
    ...
    function queryRawNotify (notify, queryOrObj, chunky) {
        ...
        var op = makeOp(dm.readall,[q, notify, native, queryObj, chunky.params, false, chunky.callback]);
        dm.execQueueOp(q, op);
    }
    ...
    function prepare(queryOrObj, paramsOrCallback, callback) {

        var notify = new dm.StreamEvents();
        var chunky = dm.getChunkyArgs(paramsOrCallback, callback);
        queryOrObj = dm.validateQuery(queryOrObj, 'query');
        function onPrepare(err, meta) {
            chunky.callback(err, meta);
            dm.nextOp(q);
        }
        
        var op = makeOp(function (callback) {
            native.prepare(notify.getQueryId(), queryOrObj, chunky.params, callback);
        }, [onPrepare]);
        dm.execQueueOp(q, op);
    }
