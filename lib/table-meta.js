...
class TableMeta {
  constructor (tableName, cols, dialect) {
    ...
    this.openCol = this.dialect === ServerDialect.Oracle ? '' : '['
    this.closeCol = this.dialect === ServerDialect.Oracle ? '' : ']'
    this.build(tableName, cols)
    this.selectArrays = this.dialect !== ServerDialect.Oracle
  }
  ...
  enclose (txt) {
    return `${this.openCol}${txt}${this.closeCol}`
  }
  ...
  getFullName () {
    const withCat = this.getWithSchemaName()
    const first = this.cols[0]
  ...
  columnList (colSubSet) {
    return colSubSet.map(e => `${this.openCol}${e.name}${this.closeCol}`).join(', ')
  }
