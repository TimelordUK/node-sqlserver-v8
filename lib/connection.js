...
// logger.setLogFile('/tmp/msnodesqlv8-trace.log')

class PrivateConnection {
  constructor (sqlMeta, userTypes, parentFn, p, cb, id) {
    this.parentFn = parentFn
    this.callback2 = cb
    logger.trace(`Creating new PrivateConnection with ID ${id}`, 'PrivateConnection.constructor')
    ...
    logger.trace(`Detected SQL Server driver version: ${this.version}`, 'PrivateConnection.constructor')
  ...
  open () {
    ...
    this.native.open(this.connectObj.conn_str, (e, c) => {
      if (e) {
        logger.error(`Connection failed for ID ${this.id}: ${e}`, 'PrivateConnection.open')
      } else {
