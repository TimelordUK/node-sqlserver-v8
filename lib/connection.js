...
class PrivateConnection {
  constructor (sqlMeta, userTypes, parentFn, p, cb, id) {
    ...
    logger.traceLazy(() => `Creating new PrivateConnection with ID ${id}`, 'PrivateConnection.constructor')
    this.native = new cppDriver.Connection()
    this.driverMgr = new driverModule.DriverMgr(this.native, this.version)
    ...
    logger.traceLazy(() => `Detected SQL Server driver version: ${this.version}`, 'PrivateConnection.constructor')
  ...
  open () {
    ...
    logger.debug(`Opening connection with ID ${this.id}`, 'PrivateConnection.open')
    logger.trace(`Connection string: ${this.connectObj.conn_str}`, 'PrivateConnection.open')
    this.native.open(this.connectObj.conn_str, (e, c) => {
      if (e) {
        logger.error(`Connection failed for ID ${this.id}: ${e.message}`, 'PrivateConnection.open')
      } else {
        logger.info(`Connection opened successfully with ID ${this.id}`, 'PrivateConnection.open')
      }
      this.queueCb(e, c)
    })
  }
...
class ConnectionWrapper {
  ...
  getUseBigIntAsNative () {
    return this.useBigIntAsNative
  }

  setUseBigIntAsNative (b) {
    this.useBigIntAsNative = b
  }
  ...
  close (immediately, callback) {
    if (this.dead) {
      logger.debug(`Connection ${this.id} already closed`, 'ConnectionWrapper.close')
      return
    }
    ...
    logger.debug(`Closing connection ${this.id}`, 'ConnectionWrapper.close')
    this.dead = true
    this.driverMgr.close(err => {
      setImmediate(() => {
        this.driverMgr.emptyQueue()
        if (err) {
          logger.error(`Error closing connection ${this.id}: ${err}`, 'ConnectionWrapper.close')
        } else {
          logger.info(`Connection ${this.id} closed successfully`, 'ConnectionWrapper.close')
        }
        callback(err, null)
      })
    })
  ...
  queryRawNotify (notify, queryOrObj, chunky) {
    ...
    if (!Object.hasOwnProperty.call(queryObj, 'bigint_as_native')) {
      queryObj.bigint_as_native = this.useBigIntAsNative
    }
    // Set up state change callback if not already set
    const stateCallback = notify.setStateChangeCallback()
    if (stateCallback) {
      queryObj.stateChangeCallback = stateCallback
    }
  ...
  query (queryOrObj, paramsOrCallback, callback) {
    ...
    logger.debug(`Executing query on connection ${this.id}`, 'ConnectionWrapper.query')
    logger.trace(`Query: ${typeof queryOrObj === 'string' ? queryOrObj : JSON.stringify(queryOrObj)}`, 'ConnectionWrapper.query')
  ...
  cancelQuery (notify, callback) {
    ...
    if (!canCancel) {
      ...
      if (notify.getHandle()) {
        ...
        let cancel = false
        notify.once('stateChange', s => {
          if (!cancel && !notify.getCancelSent() && notify.getHandle()) {
            cancel = true
            this.driverMgr.cancel(notify, (e) => {
              notify.emit('done')
              callback(e, null)
  ...
  prepare (queryOrObj, callback) {
    ...
    const onPrepare = (err, result) => {
      notify.setHandle(result.handle)
      const prepared = new PreparedStatement(this.notifier, this.driverMgr, queryObj.query_str, this.inst, notify, result)
