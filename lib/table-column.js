...
class UserTypeColumn {
  constructor (name, type, declaration, scale, precision, offset, maxLength, dialect) {
    let length = 0
    if (maxLength > 0) {
      if (type === 'nvarchar') {
        length = maxLength / 2
...
class TableColumn {
  constructor (tableName, tableSchema, ordinalPosition, columnName, type, maxLength, isPrimaryKey, dialect) {
    ...
    this.decorator = ''
    this.sql_type = this.getDeclaredType()
  }
  ...
  isReadOnly () {
    return this.is_identity ||
      ...
      (this.type === 'timestamp' && this.dialect !== ServerDialect.Oracle)
  }
  ...
  asUserType (useUTC, dialect) {
    ...
    return new UserTypeColumn(this.name, this.type, declaration, this.scale, this.precision, offset, this.max_length, dialect)
  }
