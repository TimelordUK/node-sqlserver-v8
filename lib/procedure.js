    ...
    constructor (connectionDriverMgr, procedureNotifier, theConnection, procedureMeta, procedureName, pollingEnabled, procedureTimeout) {
      ...
      function checkParamIntegrity (userParamArrayOrObject) {
        ...
        if (!Array.isArray(userParamArrayOrObject)) {
          var missing = where(meta.requiredParams, p =>
            !Object.prototype.hasOwnProperty.call(userParamArrayOrObject, p.name.slice(1)))
          if (missing.length > 0) {
            var names = missing.map(p => p.name)
            var joined = names.join()
            return `missed expected params on param object = ${joined}`
          }
        }
        return null
      }
