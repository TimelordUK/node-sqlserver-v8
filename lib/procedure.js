  ...
  class BoundProcedure {
    constructor (connectionDriverMgr, procedureNotifier, theConnection, procedureMeta, procedureName, pollingEnabled, procedureTimeout) {
      ...
      function mapParamObjectToFullParamVector (vec, meta, params) {
        ...
        // when using an object based parameter remove any input parameters that do
        // not exist on the input object will be removed - as they may b defaulted
        // and there is now easy to know if parameter can be ommitted or not.

        vec = vec.reduce((aggr, latest) => {
      ...
      function privateCall (notify, params, cb) {
        ...
        var signature = paramVec.length !== meta.params.length ? build(paramVec, name) : meta.signature
        if (cb) {
