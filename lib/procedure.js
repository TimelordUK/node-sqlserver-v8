  ...
  class BoundProcedure {
    constructor (connectionDriverMgr, procedureNotifier, theConnection, procedureMeta, procedureName, pollingEnabled, procedureTimeout) {
      ...
      function checkParamIntegrityOnParamObject (userParamObject) {
        var joinedMissingNames = getProblemParams(meta.requiredParams, p =>
          !Object.prototype.hasOwnProperty.call(userParamObject, p.name.slice(1)))
        if (joinedMissingNames) {
          return `missed expected params on param object = ${joinedMissingNames}`
        }

        var illegalMissingNames = getProblemParams(Object.keys(userParamObject), name =>
          !Object.prototype.hasOwnProperty.call(meta.paramByName, name))
