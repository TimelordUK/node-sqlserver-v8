    ...
    constructor (connectionDriverMgr, procedureNotifier, theConnection, procedureMeta, procedureName, pollingEnabled, procedureTimeout) {
      ...
      function privateCall (notify, params, cb) {
        if (meta.params.length === 0) {
          if (cb) {
            cb(new Error(`${name}: proc could not be found`, null, null))
          }
          return
        }
        if (params.length > meta.inputParams.length) {
          if (cb) {
            cb(new Error(`${name}: illegal parameter count, expected ${meta.inputCount}`), null, null)
          }
          return
        }
      ...
      function createProcedure (name, cb) {
        ...
        if (!procedure) {
          describeProcedure(name, (err, paramVector) => {
            if (!err) {
              var outputParams = paramVector.map(p => {
                return p.is_output
              })
              var inputParams = paramVector.map(p => {
                return !p.is_output
              })
