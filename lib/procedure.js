    ...
    constructor (connectionDriverMgr, procedureNotifier, theConnection, procedureMeta, procedureName, pollingEnabled, procedureTimeout) {
      ...
      function privateCall (notify, params, cb) {
        if (meta.params.length === 0) {
          const error = new Error(`${name}: proc could not be found`)
          if (cb) {
            cb(error, null, null)
          } else {
        ...
        if (params.length >= meta.params.length) {
          const error = new Error(`${name}: illegal parameter count, expected ${meta.inputCount}`)
          if (cb) {
            cb(error, null, null)
          } else {
            notify.emit('error', error)
      ...
      function createProcedure (name, cb) {
        ...
        if (!procedure) {
          describeProcedure(name, (err, paramVector) => {
            if (!err) {
              var outputParams = paramVector.reduce((agg, latest) => {
                if (latest.is_output) {
                  agg.push(latest)
                }
                return agg
              ...
              var inputParams = paramVector.reduce((agg, latest) => {
                if (!latest.is_output) {
                  agg.push(latest)
                }
                return agg
