...
const notifyModule = ((() => {
  ...
  class StreamEvents extends EventEmitter {
    ...
    setHandle (h) {
      this.handle = h
      if (this.cancelToken) {
        this.cancelQuery(this.cancelToken)
      }
    }
    ...
    setStateChangeCallback () {
      if (!this.stateChangeCallback) {
        this.stateChangeCallback = (statementId, oldState, newState) => {
          logger.debugLazy(() => `${this.queryId} state change: ${oldState} -> ${newState} (statement: ${statementId})`)
