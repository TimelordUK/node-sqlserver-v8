...
class TableBulkOpMgr {
  constructor (theConnection, user, m) {
    ...
    // node_mssql JS lib requires this property from meta
  ...
  userTypeCols () {
    ...
    return summary.columns.map(c => c.asUserType(useUTC, this.meta.dialect))
  }
  ...
  flatten (o, selectColumns, whereColumns) {
    const that = this
    const clauses = whereColumns.reduce((agg, last) => {
      if (that.hasProp(o, last.name)) {
        agg.push(`${last.name} = ${o[last.name]}`)
      }
      return agg
    }, [])
    const t = clauses.join(' and ')
    const names = selectColumns.map(c => c.name).join(', ')
    const sql = `select ${names} from ${this.meta.fullTableName} where ${t}`
    return sql
  }

  // oracle ODBC does not support select bound with array
  selectWhere (rows, callback) {
    const union = `${EOL}union all${EOL}`
    const sql = rows.map(o => this.flatten(o, this.meta.allColumns, this.meta.whereColumns)).join(union)
    this.theConnection.query(sql, callback)
  }

  selectRows (rows, callback) {
    if (this.meta.selectArrays) {
      this.selectArrays(rows, callback)
    } else {
      this.selectWhere(rows, callback)
    }
  }
