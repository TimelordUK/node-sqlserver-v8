class BulkPromises {
  constructor (bulk) {
    this.bulk = bulk
  }

  op (rows, f) {
    return new Promise((resolve, reject) => {
      try {
        f(rows, (err, res) => {
          if (err) {
            reject(err)
          }
          resolve(res)
        })
      } catch (err) {
        reject(err)
      }
    })
  }

  insert (rows) {
    return this.op(rows, (rows, cb) => this.bulk.insertRows(rows, cb))
  }

  delete (rows) {
    return this.op(rows, (rows, cb) => this.bulk.deleteRows(rows, cb))
  }

  select (rows) {
    return this.op(rows, (rows, cb) => this.bulk.selectRows(rows, cb))
  }

  update (rows) {
    return this.op(rows, (rows, cb) => this.bulk.updateRows(rows, cb))
  }
}

class TableBulkOpMgr {
  constructor (theConnection, user, m) {
    this.user = user
    this.theConnection = theConnection
    this.meta = m
    this.batch = 0
    this.usetMetaType = true
    this.summary = this.meta.getSummary()
    this.bcp = false
    this.bcpVersion = 17
    this.promises = new BulkPromises(this)
    this.useMetaType(true)
  }

  // if utc is off, we switch to using metadata from table columns
  // and on date related columns, the local timezone offset is sent
  // to driver to offset the UTC timestamp - giving ability to write local
  // dates into the database.

  asTableType (name) {
    const summary = this.meta.getSummary()
    const columns = summary.columns

    if (!name) {
      name = `${columns[0].table_name}Type`
    }
    const cols = this.userTypeCols(name)
    return new this.user.Table(name, cols)
  }

  keys (vec) {
    return vec.map(elem => {
      return this.meta.primaryCols.reduce(function (obj, column) {
        if (Object.prototype.hasOwnProperty.call(elem, column.name)) {
          obj[column.name] = elem[column.name]
        }
        return obj
      }, {})
    })
  }

  userTypeCols () {
    const summary = this.meta.getSummary()
    const useUTC = this.theConnection.getUseUTC()
    return summary.columns.map(c => c.asUserType(useUTC))
  }

  asUserType (name) {
    const summary = this.meta.getSummary()
    const columns = summary.columns
    const cols = this.userTypeCols()
    const declarations = cols.map(c => `${c.name} ${c.userType}`).join(', ')
    // CREATE TYPE TestType AS TABLE ( a VARCHAR(50), b INT );

    if (!name) {
      name = `${columns[0].table_name}Type`
    }
    return `CREATE TYPE ${name} AS TABLE (${declarations})`
  }

  // create an object of arrays where each array represents all values
  // for the batch.

  prepare () {
    const summary = this.summary
    return summary.columns.reduce((agg, col) => {
      const property = col.name
      if (Object.prototype.hasOwnProperty.call(summary.by_name, property) &&
          !summary.by_name[property].isReadOnly()) {
        agg.keys.push(property)
        if (!Object.prototype.hasOwnProperty.call(agg.arrays_by_name, property)) {
          agg.arrays_by_name[property] = []
        }
      }
      return agg
    }, {
      keys: [],
      arrays_by_name: {}
    })
  }

  arrayPerColumn (vec) {
    const res = this.prepare()
    vec.forEach(instance => {
      res.keys.reduce((agg, property) => {
        const columnValues = agg[property]
        const val = Object.prototype.hasOwnProperty.call(instance, property)
          ? instance[property]
          : null
        columnValues.push(val)
        return agg
      }, res.arrays_by_name)
    })

    return res
  }

  // if batch size is set, split the input into that batch size.

  rowBatches (rows) {
    const batches = []
    if (this.batch === 0) {
      batches.push(rows)
    } else {
      let singleBatch = []
      for (let i = 0; i < rows.length; i += 1) {
        singleBatch.push(rows[i])
        if (singleBatch.length === this.batch) {
          batches.push(singleBatch)
          singleBatch = []
        }
      }
    }

    return batches
  }

  // driver will have to recognise this is an array of arrays where each array
  // represents all values for that particular column.

  arrayPerColumnForCols (rows, colSubSet, usebcp) {
    const dataColsByName = this.arrayPerColumn(rows).arrays_by_name
    return colSubSet.reduce((agg, col) => {
      if (Object.prototype.hasOwnProperty.call(dataColsByName, col.name)) {
        const valueVector = dataColsByName[col.name]
        if (this.usetMetaType && this.meta.hasColumn(col.name)) {
          const declaration = this.meta.colByName[col.name]
          agg.push({
            value: valueVector,
            offset: declaration.offset || 0,
            sql_type: declaration.sql_type,
            precision: declaration.precision,
            scale: declaration.scale,
            bcp: usebcp,
            bcp_version: this.bcpVersion,
            ordinal_position: col.ordinal_position,
            table_name: usebcp ? this.meta.bcpTableName : ''
          })
        } else {
          agg.push(valueVector)
        }
      }
      return agg
    }, [])
  }

  // given the input array of asObjects consisting of potentially all columns, strip out
  // the sub set corresponding to the where column set.

  whereForRowsNoBatch (sql, rows, callback) {
    const colArray = this.arrayPerColumnForCols(rows, this.summary.whereColumns)
    this.theConnection.query(sql, colArray, callback)
  }

  selectRows (rows, callback) {
    const res = []
    this.whereForRowsNoBatch(this.summary.selectSignature, rows, (err, results, more) => {
      results.forEach(r => {
        res.push(r)
      })
      if (!more) {
        callback(err, res)
      }
    })
  }

  runQuery (sql, colArray) {
    const theConnection = this.theConnection
    return new Promise((resolve, reject) => {
      theConnection.query(sql, colArray, (e, res) => {
        if (e) {
          reject(e)
        } else {
          resolve(res)
        }
      })
    })
  }

  batchIterator (sql, rows, iterate) {
    return Promise.all(this.rowBatches(rows).map(b => this.runQuery(sql, iterate(b))))
  }

  insertRows (rows, callback) {
    this.batchIterator(this.summary.insertSignature, rows, b => this.arrayPerColumnForCols(b, this.summary.assignableColumns, this.bcp))
      .then(res => {
        callback(null, res)
      }).catch(e => callback(e, null))
  }

  updateRows (rows, callback) {
    this.batchIterator(this.summary.updateSignature, rows, b => this.arrayPerColumnForCols(b, this.summary.updateColumns, false)
      .concat(this.arrayPerColumnForCols(b, this.summary.whereColumns)))
      .then(res => {
        callback(null, res)
      }).catch(e => callback(e, null))
  }

  deleteRows (rows, callback) {
    this.batchIterator(this.summary.deleteSignature, rows, b => this.arrayPerColumnForCols(b, this.summary.whereColumns, false))
      .then(res => {
        callback(null, res)
      }).catch(e => callback(e, null))
  }

  getMeta () {
    return this.meta
  }

  setBatchSize (batchSize) {
    this.batch = batchSize
  }

  setWhereCols (whereCols) {
    this.meta.setWhereCols(whereCols)
    this.summary = this.meta.getSummary()
  }

  setUpdateCols (updateCols) {
    this.meta.setUpdateCols(updateCols)
    this.summary = this.meta.getSummary()
  }

  getSummary () {
    return this.meta.getSummary()
  }

  // insert only
  setUseBcp (v) {
    this.bcp = v
    if (this.bcp) {
      this.useMetaType(true)
    }
  }

  setBcpVersion (v) {
    this.bcpVersion = v
  }

  getBcpVersion () {
    return this.bcpVersion
  }

  getUseBcp () {
    return this.bcp
  }

  useMetaType (v) {
    this.usetMetaType = v
  }
}

exports.TableBulkOpMgr = TableBulkOpMgr
