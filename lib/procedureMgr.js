...
function ProcedureMgr (c, n, dm) {
  ...
  function asSelect(pv, procedure) {
    ...
    parameters.forEach(function (param) {
      if (param.is_output) {
        cmd_param = param.name + ' as ' + param.name
  ...
  var input = []
  parameters.forEach(function(param) {
    cmd_param = '@' + param.name + param.type_id
    input.push(cmd_param)
  })

  var sets = []
  parameters.forEach(function(param) {
    if (!param.is_output) {
      cmd_param = 'set @' + param.name + '=?'
      sets.push(cmd_param)
    }
  })
  ...
  parameters.forEach(function(param) {
    if (param.is_output) {
      cmd_param = '@' + param.name + ' as ' + param.name
      output.push(cmd_param)
    }
  })

  if (input.length) {
    cmd = 'declare ' + input.join(',') + ';' + sets.join(';') + ';' + cmd + ';'
  }
  if (output.length) {
    cmd += 'select ' + output.join(',') + ';'
  } */
