  ...
  class Pool extends EventEmitter {
    constructor (opt) {
      ...
      function getUserTypeTable (name) {
        ...
        return checkKilledPromise().then(() => tableMgr.promises.getUserTypeTable(name))
      }

      function getTable (name) {
        return checkKilledPromise().then(() => tableMgr.promises.getTable(name))
      }

      // returns a promise of aggregated results not a query
      function callprocAggregator (name, params, options) {
        return checkKilledPromise().then(() => aggregator.callProc(name, params, options))
      }

      function queryAggregator (sql, params, options) {
        return checkKilledPromise().then(() => aggregator.query(sql, params, options))
      }
