...
const poolModule = (() => {
  ...
  function Pool (opt) {
    ...
    function item (description, work) {
      ...
      const chunky = notifier.getChunkyArgs(work.paramsOrCallback, work.callback)
      const q = query(work.sql, work.paramsOrCallback, work.callback)
      work.poolNotifier.setQueryObj(q, chunky)
      q.on('submitted', () => {
    ...
    function checkin (description) {
      ...
      _this.emit('status', getStatus('checkin'))
    ...
    function checkout () {
      ...
      busyConnectionCount++
      _this.emit('status', getStatus('checkout'))
    ...
    function parkDescription (description) {
      ...
      promisedClose().then(() => {
        ...
        _this.emit('status', getStatus('parked'))
      }).catch(e => {
        _this.emit('error', e)
      })
      return true
