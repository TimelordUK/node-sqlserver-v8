...
const poolModule = (() => {
  ...
  class Pool extends EventEmitter {
    constructor (opt) {
      ...
      function tvp (begin, description, work) {
        description.connection.promises.getUserTypeTable(work.sql)
          .then(res => {
            work.callback(null, res)
          ...
          .catch(err => {
            work.callback(err, null)
            if (_this.listenerCount('error') > 0) {
              _this.emit('error', err, false)
            }
            setImmediate(() => {
              crank()
            })
      ...
      function getUserTypeTable (name) {
        const p = new Promise((resolve, reject) => {
          const notifier = new PoolEventCaster()
          enqueue(newWorkItem(name, null, (err, res) => {
            if (err) {
              reject(err)
            } else {
              resolve(res)
            }
          }, notifier, workTypeEnum.TVP))
        })
        crank()
        return p
      }
