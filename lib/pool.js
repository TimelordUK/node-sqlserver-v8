...
const poolModule = (() => {
  ...
  class PoolOptions {
    constructor (opt) {
      ...
      this.useBigIntAsNative = this.getOpt(opt, 'useBigIntAsNative', null)
    ...
    constructor (opt) {
      ...
      function promotePause () {
        ...
        for (let i = 0; i < pause.length; i++) {
          if (!pause[i].poolNotifier.isPaused()) {
      ...
      function checkin (activity, description) {
        ...
        // A description must never appear twice in idle. If it did, park() could null
        // its connection via one reference while the duplicate is still checked out
        // and used - yielding "Cannot read properties of null (reading 'query')".
        if (idle.indexOf(description) >= 0 || parked.indexOf(description) >= 0) {
          const dupMsg = `[${description.id}] ignoring duplicate checkin from '${activity}'`
          _this.emit('debug', dupMsg)
          logger.debugLazy(() => `${dupMsg} - ${getPoolStateContext()}`, 'Pool.checkin')
          return
        }
      ...
      async function grow () {
        ...
        function connectionOptions (c) {
          ...
          if (options.useBigIntAsNative === true || options.useBigIntAsNative === false) {
            c.setUseBigIntAsNative(options.useBigIntAsNative)
          }
        }
      ...
      function heartbeat () {
        ...
        // a failed statement can raise both 'error' and 'done' (see reader.js dispatch
        // error path). Only the first of the two may return the description to the pool.
        let settled = false
        q.on('column', (i, v) => {
      ...
      function close (cb) {
        ...
        // invoke close - mapping the method itself leaves idle connections open
        const toClosePromise = idle.map(description => description.connection.promises.close())
        // mark closed synchronously so work submitted during shutdown is rejected
        closed = true
