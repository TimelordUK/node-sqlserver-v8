  ...
  function Pool (opt) {
    ...
    function check () {
      ...
      if (firstIndex > -1) {
        ...
        q.on('done', () => {
          description.lastActive = new Date()
          _this.emit('debug', `[${description.id}] keep alive response = ${description.heatbeatSqlResponse} ${description.lastActive} ${description.keepAliveCount} idle length ${idle.length} busy count ${busyConnectionCount}`)
        })
        q.on('error', (e) => {
          _this.emit('error', e)
