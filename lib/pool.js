...
const poolModule = (() => {
  ...
  class Pool extends EventEmitter {
    constructor (opt) {
      super()
      const clientPromises = connectionModule.promises
      const idle = []
      const parked = []
      const workQueue = []
      const pause = []
      ...
      function runTheQuery (begin, q, description, work) {
      ...
      function getTheQuery (description, work) {
        ...
        switch (work.workType) {
          case workTypeEnum.QUERY:
            q = description.connection.query(work.sql, work.paramsOrCallback, work.callback)
            break

          case workTypeEnum.RAW:
            q = description.connection.queryRaw(work.sql, work.paramsOrCallback, work.callback)
            break

          case workTypeEnum.PROC:
            q = description.connection.callproc(work.sql, work.paramsOrCallback, work.callback)
            break
        }
        return q
      }
      ...
      function item (description, work) {
        ...
        if (q) {
          runTheQuery(begin, q, description, work)
        }
      }
