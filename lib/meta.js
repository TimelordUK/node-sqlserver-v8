  ...
  function Meta () {
    ...
    function getTableDefinition (conn, mapFn, callback) {
      getServerVersionRes(conn).then(serverRes => {
        console.log(JSON.stringify(serverRes, null, 4))
        var schema2014OrOlder = serverRes.MajorVersion <= 2014.0
        var j = `{
        "MajorVersion": 2014,
        "ProductLevel": "SP1",
        "Edition": "Express Edition (64-bit)",
        "ProductVersion": "12.0.4100.1"
    }`
        var p = JSON.parse(j)
        const fileName = schema2014OrOlder ? 'table_describe.2014.sql' : 'table_describe.sql'
        console.log(`fileName ${fileName} schema2014OrOlder ${schema2014OrOlder}`)
