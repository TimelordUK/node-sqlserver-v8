  ...
  function Meta () {
    ...
    function getUserType (conn, userTypeName, callback) {
      resolveTableTypeSql(function (sql) {
        var schemaName = 'dbo'
        var unqualifiedTableName = userTypeName
        var schemaIndex = userTypeName.indexOf(".")
        if (schemaIndex > 0) {
          schemaName = userTypeName.substr(0,schemaIndex)
          unqualifiedTableName = userTypeName.substr(schemaIndex + 1)
        }
    ...
    function getProcedureDefinition (conn, procedureName, callback) {
