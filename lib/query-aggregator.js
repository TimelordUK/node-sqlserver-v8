class AggregatorResults {
  ...
  onColumn (c, v) {
    ...
    if (global.msnodesqlv8Logger && global.msnodesqlv8Logger.isLevelEnabled('TRACE')) {
      const colName = meta[c] ? meta[c].name : `column${c}`
      const valueInfo = v === null ? 'null' : (typeof v === 'string' ? `string(${v.length})` : typeof v)
      global.msnodesqlv8Logger.log('TRACE', `[query-aggregator.onColumn] Column ${c} (${colName}): ${valueInfo}`)
    }
