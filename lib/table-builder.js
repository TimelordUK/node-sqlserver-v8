...
class TableBuilder {
  ...
  getDrop (tableName) {
    ...
    } else if (this.dialect === ServerDialect.Oracle) {
      dropTableSql = `BEGIN EXECUTE IMMEDIATE 'DROP TABLE ${tableName}'; 
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN
         RAISE;
...
END;`
  ...
  compute () {
    const openCol = this.dialect === ServerDialect.Oracle ? '' : '['
    const closeCol = this.dialect === ServerDialect.Oracle ? '' : ']'
    const tableName = this.fullTableName
    const fullTypedColumns = this.columns.map(e => `${openCol}${e.name}${closeCol} ${this.typed(e)}`).join(', ')
    ...
    const columnNames = this.columns.map(e => `${openCol}${e.name}${closeCol}`).join(', ')
    const dropTableSql = this.getDrop(tableName)
    const createTableSql = `CREATE TABLE ${tableName} (${fullTypedColumns})`
    const insertColumns = this.columns.filter(c => !c.isReadOnly())

    const typedColumns = insertColumns.map(e => `${openCol}${e.name}${closeCol} ${e.procTyped()}`).join(', ')
