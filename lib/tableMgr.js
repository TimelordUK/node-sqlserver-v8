...
/*
todo:
    1. delete via primary key - or allow user specified where columns.
...
exports.tableMgr = function (c) {
    ...
    function describe(name, cb) {
        ...
        if (meta == null) {
            describeTable(name, function (err, cols) {
                var insertSignature = buildInsertForColumns(name, cols);
                var wherePrimarySignature = buildWhereForPrimaryKeyColumns(cols);
                var deletePrimarySignature = buildDeleteForPrimaryColumns(name,cols);
                var colByName = {};
                for (var c = 0; c < cols.length; ++c) {
                    colByName[cols[c].name] = cols[c];
                }
                var meta = {
                    where_primary_signature : wherePrimarySignature,
                    insert_signature : insertSignature,
                    delete_primary_signature : deletePrimarySignature,
                    primary_columns : primaryCols(cols),
                    columns : cols,
                    by_name : colByName
                };
                cache[name] = meta;
    ...
    function buildDeleteForPrimaryColumns(name,cols) {
        var sql = 'delete from ' + name + ' ';
        sql += buildWhereForPrimaryKeyColumns(cols);
        return sql;
    }

    function primaryCols(cols) {
        var primaryKeyCols = [];
    ...
    function buildWhereForPrimaryKeyColumns(cols) {
        var pcols = primaryCols(cols);
        var where = buildWhereForColumns(pcols);
        return where;
    }

    function buildWhereForColumns(cols) {
        var sql = "where ( ";
        cols.forEach(function (col) {
            sql += col.name;
            sql += ' = ?, ';
    ...
    function buildInsertForColumns(name, cols) {
    ...
    function bulkTableOpMgr(n, m) {
        ...
        // need to be able to delete via user specified key array .... right now only works off primary.

        function deleteRows(rows, callback) {
            var batches = rowBatches(rows);
            var sql = meta.delete_primary_signature;
            var b = 0;
            ...
            function iterate() {
                ...
                var primaryCols = meta.primary_columns;
                var colArray = [];
