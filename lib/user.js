...
// sql.DateTime2(value, [scale]) -- optional scale definition
     ...
     function DateTime2(p, scale, fraction) {
        if (!fraction && p) fraction = p.getUTCMilliseconds();
        return {
            sql_type: SQL_TYPE_TIMESTAMP,
            value: p,
            fraction: fraction,
...
// datetime Date round to 10 ms as fraction is not guaranteed
    ...
    function TzOffsetQuery(s, offsetMinutes) {
        return {
            ...
            query_timeout: 0,
            query_polling: false,
            query_tz_adjustment : offsetMinutes ? offsetMinutes : - new Date().getTimezoneOffset()
        }
    }
    ...
    function TimeoutQuery(s, t_secs) {
        return {
            query_str: s,
            query_timeout: t_secs,
            query_polling: false,
            query_tz_adjustment:0
        }
    }
