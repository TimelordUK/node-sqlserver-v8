...
const driverModule = ((() => {
  ...
  function DriverMgr (sql) {
    ...
    function execCancel (qid, queueItem, callback) {
      ...
      const cb = args[3]
      if (queueItem.operationId === workQueue.peek().operationId) {
    ...
    function readAllPrepared (notify, queryObj, params, cb) {
      notify.setOperationId(workQueue.enqueue(driverCommandEnum.QUERY,
        (notify, query, params, callback) => {
          setImmediate(() => {
            let q = reader.getQuery(notify, query, params, {
    ...
    function readAllQuery (notify, queryObj, params, cb) {
      notify.setOperationId(workQueue.enqueue(driverCommandEnum.QUERY, (notify, query, params, callback) => {
        setImmediate(() => {
          let q = reader.getQuery(notify, query, params, {
