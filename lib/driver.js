...
var driverModule = (function () {
  ...
  function DriverMgr (sql) {
    ...
    function procedureUnbindParameters (queryId, outputParams, callback, results, more) {
      workQueue.enqueue(driverCommandEnum.UNBIND, function () {
        cppDriver.unbind(queryId, function (err, outputVector) {
          if (err && callback) {
            callback(err, results)
          }
          cbFreeStatement(queryId, outputVector, callback, results, more)
        })
        if (!more) {
          workQueue.nextOp()
        }
    ...
    function prepare (notify, queryOrObj, callback) {
      workQueue.enqueue(driverCommandEnum.PREPARE, function () {
        prepareQuery(notify, queryOrObj, function onPrepare (err, meta) {
          callback(err, meta)
          workQueue.nextOp()
        })
      }, [])
