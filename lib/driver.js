              ...
              // statements are completed
                ...
                if (!endMore) {
                  workQueue.enqueue(driverCommandEnum.UNBIND, (a) => {
                    cppDriver.unbind(not.getQueryId(), (err, outputVector) => {
                      if (err && callback) {
                        callback(err, results)
                      }
                      onStatementComplete(notify, outputVector, callback, results, a)
                      if (!a) {
                        workQueue.nextOp()
                      }
                    })
                  ...
                  onStatementComplete(notify, null, callback, results, endMore)
                }
              }
            }, callback)
            notify.setQueryWorker(q)
